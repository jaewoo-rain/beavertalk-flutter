...
class NetworkErrorView extends StatelessWidget {
  ...
  /// Overrides the 홈으로 destination. Defaults to unwinding to the [AuthGate]
  /// root, which is what renders [Routes.home]. Ignored unless [showHome].
  ...
  // Keep the AuthGate root (popUntil isFirst, not a push that removes it):
  // AuthGate is what swaps to login/home on auth changes, so removing it would
  // strand a later session-expiry with no redirect. And popping is what "go
  // home" *means* here — AuthGate already renders HomeScreen, so pushing
  // Routes.home on top of it stacked two homes and left a back press showing
  // home again.
