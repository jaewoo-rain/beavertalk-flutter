/// 음소(자모) → 조음 도해 자산 매핑.
///
/// 도해는 Figma `Articulatory diagrams` 5개 섹션을 그대로 내보낸 것이다
/// (`assets/articulatory/` 75종 · 1.5배 PNG). 다크 카드 위에 발광 기류를 그린
/// 그림이라 밝은 시트 위에 얹어도 대비가 산다.
///
/// Figma 쪽은 84종이다. 차이 9종은 둘로 갈린다 —
/// `ㅁ(coda)`·`ㅇ(coda)` 2종은 초성판과 같은 그림이라 안 가져왔고(아래 [_airflowCoda]),
/// 모음 hold 신규 7종은 띄울 신호가 없어서다. hold 는 「자세를 유지하라」는 뜻이라
/// 지속 구간 신호가 있어야 하는데 `phoneme_misses` 는 자모 치환만 담는다.
/// 같은 이유로 `sy_*` 5종(모음 삽입 = 음절 수 증가)도 매핑이 없다.
///
/// 🔴 앱의 `vw_*_hold` 3종은 Figma 재설계(2026-08-30) 이전 판이다. 매핑이 없어
///    화면에는 안 나오지만 자산으로는 어긋나 있다. hold 를 쓰게 되는 날 10종을
///    통째로 다시 받아야 한다 — 3종만 갱신하면 세트가 반쪽이 된다.
///
