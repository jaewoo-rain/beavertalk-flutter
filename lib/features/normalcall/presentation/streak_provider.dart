...
/// 최대 쪽 수 — 100건 × 4 = 400건. 하루 1통화면 1년 넘게 센다.
/// ⚠ 하루 여러 통화하는 사용자는 **적게 센다** — 하루 합산 한도 안에서는 횟수 제한이
///   없어서(09-23) 매일 여러 번 걸면 400건이 금방 찬다. 신서버는 달력 API 의
///   `streak_days` 를 쓰므로 이 계산은 구서버 폴백·최고 기록에만 남는다(남은판단 S4·P24-a).
const _maxPages = 4;
