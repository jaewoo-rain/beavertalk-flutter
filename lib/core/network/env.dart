...
abstract final class Env {
  ...
  ///
  /// ## 앱 서버에 라우트가 있다(09-26 정정)
  ///
  /// 앱 서버는 `d269bb6`(2026-07-22)부터 `@router.websocket("/pron/stt/ws")` 를 갖고
  /// 있다(`domains/learning/realtime/ws_router.py`). 토큰이 없거나 무효면 **수락 전에**
  /// 닫으므로 핸드셰이크가 **403** 으로 끝난다.
  ///
  /// ⚠ 이 주석은 한때 「09-08 실측: 앱 백엔드 403 = 라우트 없음」 이라고 적었다 — **틀렸다.**
  /// 토큰 없는 핸드셰이크는 `/calls/stream` · `/pron/stt/ws` · 없는 경로 모두 403 이라
  /// 403 으로는 라우트 유무를 가를 수 없다(QA F027 · 09-26 재실측). GET 도 마찬가지로
  /// WebSocket 라우트에는 항상 404 다. 라우트를 확인하려면 **유효 토큰**으로 핸드셰이크해
  /// 101 을 봐야 한다 — 앱 서버에서 그 101 은 아직 실측하지 않았다(STT 과금 경로).
  ///
  /// 이 키가 비어 있으면 [apiBaseUrl](지금은 demo-api)로 붙는다.
  ///
  /// ⛔ **기본값을 웹 백엔드로 바꾸지 마라.** STT 를 앱 서버로 옮긴 것은 「토큰
